...
| Provider diversity/conformance | P5/ADOPT-CONCEPT | T500 | `nh_media.providers` | five-kind success/error/cancel/timeout/fallback/redaction suite | implemented |
| TTS/narration/cache | P5/REIMPLEMENT | T511 | `nh_media.speech.tts` | voice snapshot, full fingerprint, WAV ProducedBlob→Artifact | implemented |
| ASR/backend selection | P5/REIMPLEMENT+ADOPT | T512 | `nh_media.speech.asr` | typed transcript, explicit fake backend and precision contract | implemented |
...
| Scene detection/features | P5/REIMPLEMENT | T520 | `nh_media.video.scenes` | source revision, ranges, zero-scene policy | implemented |
...
| Text embeddings | P6/REIMPLEMENT | T523 | `nh_media.matching.embeddings` | model/dimension/normalization index and reload boundary | implemented |
...
| Timeline/video composition | P4–P5/REIMPLEMENT | T530 | Go `internal/render` | exact Timeline/Profile deterministic allowlisted plan | implemented |
...
| Rendering/deliverable QA | P5/REIMPLEMENT | T531/T531-S1 | Go `internal/render` | real FFmpeg output, ffprobe QA, incomplete output rejected | implemented |
