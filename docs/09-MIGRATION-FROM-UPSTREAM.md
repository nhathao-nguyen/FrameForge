...
| REST daemon | stdlib HTTP TaskAPIServer, X-API-Key, tasks/batches/schedules/DLQ/artifacts/health. | FastAPI Product API + auth/domain resources. | `REPLACE` product surface for boundary/security reasons; retain V1 listener/gateway. | Route mapper `/tasks...` to V2 or frozen daemon behind internal boundary. | OpenAPI snapshots, route/status/result/auth/limits/filename guards. | Compatibility clients migrated; explicit breaking-change release approved. |
