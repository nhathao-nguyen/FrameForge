...
`OPEN`; OQ-12–OQ-15 were decided by the owner on 2026-08-16. OQ-13 has a superseding owner decision:
Go owns the V2 control plane, while Python is isolated to ML/AI and frozen V1 compatibility. This
mirror exists so task handoffs expose blockers without changing the source. No deadline has been
assigned in the source, so the deadline column deliberately says `not set`.
...
| OQ-13 | DECIDED | Go Product/control plane; Go media workers preferred; isolated Python 3.12 ML/V1 workloads with language-neutral contracts | resolved for topology/T002; ML 3.13 move requires parity evidence | repository owner | 2026-08-16 |
