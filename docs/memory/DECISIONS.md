...
| D-T000-012 | 2026-08-16 | repository owner | Use `nh_media` for new Python V2 compute/ML; preserve `movie_narrator`; reject `your_engine`, `video_engine` and `frameforge` as Python public namespaces. Go uses `github.com/nhathao-nguyen/FrameForge` conventions and internal packages; contracts remain language-neutral. | Placeholder/generic/product-name namespaces. | T100 and new Python imports use `nh_media`; the legacy adapter keeps its upstream namespace; Go does not expose a global public `frameforge` package. |
...
| D-T000-013-SUPERSEDING | 2026-08-16 | repository owner | V2 Product API/control plane is Go with no Python dependency; prefer Go media/FFmpeg workers; isolate Python 3.12 to ML/AI and frozen V1. Go/Python communication uses versioned language-neutral contracts; implementation language is replaceable; long work follows a bounded durable Job/outbox/worker/result flow. | Python Product API; big-bang Go rewrite; framework-specific serialization. | T002 pins Go plus isolated Python/FFmpeg environments; T100/T106 use Go control-plane boundaries; parity, contract and rollback evidence remain mandatory. |
