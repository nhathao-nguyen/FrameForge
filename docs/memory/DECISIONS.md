...
| D-T000-012 | 2026-08-16 | repository owner | Use `nh_media` as the V2 Python/package namespace; reject `your_engine`, `video_engine` and `frameforge` as public V2 namespaces for now; preserve `movie_narrator`. | Placeholder/generic/product-name namespaces. | T100 and all new imports use `nh_media`; legacy adapter keeps its upstream namespace. |
| D-T000-013 | 2026-08-16 | repository owner | Product/API/core use Python 3.13; frozen legacy/ML image uses Python 3.12 temporarily with separate locks/images until Phase 0 dependency/ML/CUDA parity. | 3.13 everywhere; 3.12 everywhere. | Two explicit runtime boundaries must be tested and later merge requires evidence. |
