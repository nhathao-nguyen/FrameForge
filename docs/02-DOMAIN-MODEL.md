...
Tài liệu này định nghĩa ngôn ngữ domain canonical của NH-Media. Tên trạng thái phải được dùng
nguyên văn ở PostgreSQL, REST và event payload. Upstream task/status names appear only in research
evidence and never cross a product boundary.
...
| **Analysis** | Versioned result from media/speech/vision/style analysis. | `analysis_id`; belongs to Project and exact input refs. | `created → running → completed|failed|cancelled`; immutable result after completion. | References source Artifacts, producer JobStep, model/provider snapshot and output Artifact/document. | Request snapshot immutable; terminal output/provenance immutable. | `analyses`; baseline table or typed Artifact manifest as specified per analysis kind. |
| **ReferenceStyleAnalysis** | Abstract production traits from a user-authorized reference: pacing, shot duration, narration/subtitle/framing/music/rhythm characteristics. | `analysis_id` with `kind=reference_style`; Project-scoped. | Follows Analysis lifecycle. | References input Asset/Artifact and produces typed metrics, never copied footage. | Completed metrics/provenance immutable; user labels create a derived version. | Analysis metadata + Artifact; later phase. |
...
| **GenerationCandidate** | One alternative output produced for a generation request. | `candidate_id`; belongs to Project/JobStep and a candidate group. | `created → generated → evaluated → selected|rejected|superseded`. | References generated ScriptVersion/Artifact/domain proposal and EvaluationResults. | Candidate payload immutable after generated; selection is separate audited state. | `generation_candidates`; introduced with candidate workflows. |
| **EvaluationResult** | Versioned scoring/evidence for a GenerationCandidate. | `evaluation_result_id`; belongs to candidate and policy version. | `created → completed|failed`. | Records score components, evaluator/model provenance and explanation Artifact. | Completed result immutable. | `evaluation_results`; later phase. |
| **CandidateSelectionPolicy** | Versioned rules for choosing among candidates. | `selection_policy_id`; system/Workspace scoped. | `draft → active → deprecated → disabled`. | Snapshot by candidate-generating JobStep. | Immutable after activation. | `candidate_selection_policies`; later phase. |
...
- Thuật ngữ `stage` chỉ dùng trong mô tả tự nhiên hoặc research. Contract NH-Media dùng `node`
  (definition) và `job step` (execution).
